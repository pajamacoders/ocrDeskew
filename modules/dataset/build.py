...
def build_dataloader(type, train_batch, valid_batch, train, valid, augment_fn):
    ...
    train_loader = DataLoader(train_dataset, batch_size=train_batch, shuffle=(sampler is None), sampler=sampler, num_workers=16, pin_memory=True, drop_last=True)
    valid_dataset = ocrFactory[type](**valid, transformer=augment_fn)
    valid_loader = DataLoader(valid_dataset, batch_size=valid_batch, shuffle=True, num_workers=16, pin_memory=True, drop_last=True)
