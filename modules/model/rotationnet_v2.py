...
class RotationNet_v2(nn.Module):
    def __init__(self, pretrained=None):
        super(RotationNet_v2, self).__init__()
        self.blocks = torch.nn.Sequential(BasicBlock(1,16,5), #256x256
            BasicBlock(16,32,5), #128x128
            BasicBlock(32,64,3), #64x64
            BasicBlock(64,128,3) #32x32
        ) 
        self.avg_layer = nn.AvgPool2d((32,32))
        self.fc = nn.Sequential(nn.Linear(128,64),nn.BatchNorm1d(64),nn.ReLU(inplace=True),nn.Linear(64,1))
        self.__init_weight()
        if pretrained:
    ...
    def forward(self, x):
        out = self.blocks(x)
        out = self.avg_layer(out).squeeze(-1).squeeze(-1)
