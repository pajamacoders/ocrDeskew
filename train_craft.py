...
def train(model, loader, fn_cls_loss, key_target, optimizer, mllogger, step, prediction_parser=None, craft=None):
    model.train()
    avg_total_loss = 0
    avg_cls_loss = 0
    num_samples=0
...
if __name__ == "__main__":
    ...
    for step in range(max_epoch):
        train(model, train_loader, fn_cls_loss, key_metric, opt, mltracker, step, prediction_parser, craft=craft)
        if (step+1)%valid_ecpoh==0:
            valid(model, valid_loader, fn_cls_loss, key_metric,  mltracker, step, vis_func, prediction_parser, craft=craft)
        lr_scheduler.step()
